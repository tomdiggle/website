---
title: Objective Cloud
date: '2013-01-21'
duration: '1:13:15'
description:
categories: [development]
tags: [cloud, service]
url: /80-objective-cloud
attachments: ["http://media.blubrry.com/nsbrief/nsbrief.com/episodes/nsbrief_80_objective_cloud.m4a"]
episode_number: 80
---

In this episode, we chat with Chris and Amin, developers of the upcoming cloud service, [Objective Cloud](http://objective-cloud.com).

##  Show Notes
- [Objective Cloud](http://objective-cloud.com)
- [iDeveloperLive: Objective C in the Cloud](http://blog.ideveloper.tv/podcast74/)
- [WebObjects](http://en.wikipedia.org/wiki/WebObjects)
- [Store News](http://store-news-app.com/)
- [curl](http://en.wikipedia.org/wiki/CURL)
- [unit tests is teh suck, urr](http://blog.wilshipley.com/2005/09/unit-testing-is-teh-suck-urr.html)
- [NSCoding](https://developer.apple.com/library/mac/#documentation/Cocoa/Reference/Foundation/Protocols/NSCoding_Protocol/Reference/Reference.html)
- [NSIncrementalStore](https://developer.apple.com/library/mac/#documentation/CoreData/Reference/NSIncrementalStore_Class/Reference/NSIncrementalStore.html)
