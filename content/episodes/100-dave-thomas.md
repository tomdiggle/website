---
title: Dave Thomas
date: '2013-07-01'
duration: '44:05'
description: 
categories: ['Development']
tags: ['Languages']
url: /100-dave-thomas
episode_number: 100
attachments: ["http://media.blubrry.com/nsbrief/nsbrief.com/episodes/nsbrief_100_dave_thomas.m4a"]
---

[Dave Thomas](http://gotocon.com/amsterdam-2013/speaker/Dave+Thomas) is an adjunct research professor at Carleton University, and the University Of Queensland and is widely published in the software engineering literature. In this episode, we talk a walk down memory lane and look at languages past and present and compare them to Objective C.


## Show Notes
### General Topics
- [Boxing](http://en.wikipedia.org/wiki/Boxing_(programming)
- [Delay Slots](http://en.wikipedia.org/wiki/Delay_slot)
- [Lexical Closures](http://en.wikipedia.org/wiki/Lexical_closures)
- [Generics](http://en.wikipedia.org/wiki/Generics_in_Java)
- [Dynamic Dispatch](http://en.wikipedia.org/wiki/Dynamic_dispatch)
- [Double Dispatch](http://en.wikipedia.org/wiki/Double_dispatch)
- [Multimethods](http://en.wikipedia.org/wiki/Multimethods)

### People
- [Alan Kay](http://en.wikipedia.org/wiki/Alan_Kay)
- [Brad Cox](http://en.wikipedia.org/wiki/Brad_Cox)
- [Dan Ingalls](http://en.wikipedia.org/wiki/Dan_Ingalls)

### Languages
- [C++](http://en.wikipedia.org/wiki/C%2B%2B)
- [C#](http://en.wikipedia.org/wiki/C_Sharp_(programming_language)
- [Java](http://en.wikipedia.org/wiki/Java_programming)
- [Objective-C](http://en.wikipedia.org/wiki/Objective-C)
- [Forth](http://en.wikipedia.org/wiki/Forth_(programming_language)
- [Scheme](http://en.wikipedia.org/wiki/Scheme_(programming_language)
- [Smalltalk](http://en.wikipedia.org/wiki/Smalltalk)
- [Scala](http://en.wikipedia.org/wiki/Scala_(programming_language)
- [Racket](http://en.wikipedia.org/wiki/Racket_(programming_language)
- [APL](http://en.wikipedia.org/wiki/APL_programming_language)
- [APL Keyboard](https://en.wikipedia.org/wiki/File:APL-keybd2.svg)
- [Azul](http://en.wikipedia.org/wiki/Azul_Systems)
- [Lisp](http://en.wikipedia.org/wiki/Lisp_(programming_language)
- [Vector Langauges](http://en.wikipedia.org/wiki/Array_programming)
- [Logic Languages](http://en.wikipedia.org/wiki/Logic_programming)
- [J](http://en.wikipedia.org/wiki/J_(programming_language)
- [K](http://en.wikipedia.org/wiki/K_(programming_language)
- [Ada](http://en.wikipedia.org/wiki/Ada_programming)
- [F-Script](http://en.wikipedia.org/wiki/F-Script_(programming_language)
- [SNOBOL](http://en.wikipedia.org/wiki/SNOBOL_programming_language)
- [Icon](http://en.wikipedia.org/wiki/Icon_(programming_language)
- [Prolog](http://en.wikipedia.org/wiki/Prolog)
- [Datalog](http://en.wikipedia.org/wiki/Datalog)
- [Erlang](http://en.wikipedia.org/wiki/Erlang_(programming_language)
- [Actor](http://en.wikipedia.org/wiki/Actor_(programming_language)
- [Constraint Language](http://en.wikipedia.org/wiki/Object_Constraint_Language)
- [Closure](http://en.wikipedia.org/wiki/Closure_(computer_science)
- [awk](http://www.gnu.org/software/gawk/manual/gawk.html)
- [sed](http://www.gnu.org/software/sed/)
- [Actra](http://en.wikipedia.org/wiki/ACTRA)
- [QNX](http://en.wikipedia.org/wiki/QNX)
- [Elixir](http://en.wikipedia.org/wiki/Elixir_(programming_language)
- [Programming Elixir](http://pragprog.com/book/elixir/programming-elixir) - Pragmatic Programmers
- [Introducing Elixir](http://shop.oreilly.com/product/0636920030584.do) - Oreilly
 
{{> audio_mastered_jonathan_penn_cocoa_manifest_blog }}
